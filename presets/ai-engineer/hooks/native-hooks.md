...
## ai-engineer domain hooks

- **Prompt Change Eval** — When a system prompt, agent definition, or model id changes, re-run the offline eval set against `evals/thresholds.yaml`. (`fileEdited`)
- **Tool Contract Check** — When an agent or MCP tool is added, check its schema, description, error contract, and IAM scope. (`fileCreated`)
- **Agent Card Update** — Manual agent-card refresh capturing tools, guardrails, eval results, limitations, and cost. (`userTriggered`)
