...
- **Prompt Change Eval** — When a system prompt, agent definition, or model id changes, re-run the offline eval set before it ships. (`PostFileSave`)
- **Tool Contract Check** — When an agent tool or MCP tool definition is added, check its schema, description, and error contract. (`PostFileCreate`)
...
- `/agent-card-update` — Run on demand to refresh the agent card with tools, guardrails, eval results, limitations, and cost.
