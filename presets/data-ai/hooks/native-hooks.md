...
- **Experiment Log** — When a notebook or training script changes, append a structured experiment log entry. (`PostFileSave`)
- **Data Validation** — When a new dataset or loader is added, generate a schema/validation check. (`PostFileCreate`)
...
- `/model-card-update` — Run on demand to refresh the model card with metrics, data, and intended use.
