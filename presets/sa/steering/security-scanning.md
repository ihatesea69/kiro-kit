...
This workspace has three security assets with **non-overlapping** scopes. Pick by
scope, not by convenience, and never report the same issue through two of them.
...
| `security-auditor` agent | Infrastructure, containers, IAM, CI/CD, compliance | Medium | IaC review, CIS/SOC2/PCI audits, image CVEs |

Rules of thumb:
- A vulnerability in application source code → deep scan.
- A misconfiguration in Terraform/Kubernetes/IAM/pipelines → `security-auditor`.
