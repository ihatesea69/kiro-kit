...
description: Methodology, severity taxonomy, templates, and CI contract for a whole-repository deep security scan. Activate when running /security:deep-scan or when orchestrating the recon → find → validate → judge → report pipeline.
---

# Deep Security Scan

Activate this skill when running a whole-repository deep security scan (the
`/security:deep-scan` command). It holds the methodology the security agents
...
- You are orchestrating or acting as one of the `security-recon`, `security-finder`,
  `security-triage`, `security-validator`, `security-judge`, or `security-reporter`
  agents.
...
- **Infrastructure / compliance audit** (IAM, container CVEs, CIS/SOC2) → use the
  `security-auditor` agent. This scan owns application *code* vulnerabilities.
- Avoid double-reporting: if a finding is purely an infra misconfiguration, defer it
  to `security-auditor`.
...
```
recon (1, serial)  →  finders (N, parallel)  [+ semgrep → triage (N, parallel), optional]
   →  validators (1 per candidate, parallel)  →  judge (1, serial dedup)
   →  reporter (1, serial)
```
...
## Optional: Semgrep hybrid mode

With `--semgrep`, Semgrep generates candidates first and `security-triage` agents
classify each one true/false positive against real source context (the QASecClaw
pattern). Survivors go through the SAME validator and the SAME ≥8/10 gate — no
shortcut. It augments the manual finders, never replaces them: rules cannot see
business-logic authorization flaws or multi-hop data flows. Degrade silently when
Semgrep is absent. See `references/semgrep-hybrid.md`.

## Re-scans

When a previous scan exists, the judge matches findings by ROOT CAUSE (not slug
string, not line number) and reuses the previous slug, so a finding keeps one
identity for its whole life. Each finding is marked `new`, `persisting`, or
`regressed`, and the reporter writes `delta.md`. On a scoped re-scan, only findings
inside the scanned scope may be marked `fixed` — anything outside was not examined
...
- `references/semgrep-hybrid.md` — optional SAST-first candidate generation.
- `references/ci-integration.md` — the `findings.json` CI contract and the gate script.

## Scripts & assets

- `scripts/check-findings.mjs` — zero-dependency CI gate; exits non-zero on open
  findings at or above a threshold (`--fail-on`, `--max-age-days`, `--format github`).
- `assets/deep-scan-gate.yml` — GitHub Actions workflow template wrapping it.
...
`scan-manifest.json` · `findings.json` · `coverage.json` · `delta.md` (re-scans
only). See the `security-scanning` steering file for the workspace contract and CI
consumption.
