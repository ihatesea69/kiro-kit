...
3. **.kiro/.env file**
   ```bash
   echo 'GEMINI_API_KEY=your-api-key' > .kiro/.env
   ```

4. **.kiro/skills/.env file** (shared across all Gemini skills)
   ```bash
   echo 'GEMINI_API_KEY=your-api-key' > .kiro/skills/.env
   ```

5. **Skill directory `.env` file**
   ```bash
   echo 'GEMINI_API_KEY=your-api-key' > .kiro/skills/your-skill/.env
   ```
