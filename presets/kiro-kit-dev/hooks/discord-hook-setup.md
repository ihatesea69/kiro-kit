...
The Discord hook (`send-discord.sh`) sends rich embedded messages to a Discord channel when Kiro completes implementation tasks. Messages include session time, project info, and task summaries.
...
3. Click **"New Webhook"**
4. Configure webhook:
   - **Name:** `Kiro Bot` (or your preference)
...
2. **.kiro/.env** - Project-level Claude configuration
3. **.kiro/hooks/.env** - Hook-specific configuration
...
**Option B: `.kiro/.env`** (project-level override):
```bash
DISCORD_WEBHOOK_URL=https://discord.com/api/webhooks/YOUR_WEBHOOK_ID/YOUR_WEBHOOK_TOKEN
```

**Option C: `.kiro/hooks/.env`** (hook-specific):
```bash
DISCORD_WEBHOOK_URL=https://discord.com/api/webhooks/YOUR_WEBHOOK_ID/YOUR_WEBHOOK_TOKEN
```
...
```bash
chmod +x .kiro/hooks/send-discord.sh
```
...
```bash
./.kiro/hooks/send-discord.sh 'Test notification from Kiro'
```
...
```bash
./.kiro/hooks/send-discord.sh 'Task completed: Added user authentication feature'
```

**With multi-line messages:**
```bash
./.kiro/hooks/send-discord.sh 'Implementation Complete
...
```bash
./.kiro/hooks/send-discord.sh 'Fixed bug in user'\''s profile page'
```

### Automated Usage (Kiro Workflow)

Claude automatically calls this script when completing implementations. This is configured in `.kiro/workflows/development-rules.md`:

```markdown
- When you finish the implementation, send a full summary report to Discord channel
  with `./.kiro/hooks/send-discord.sh 'Your message here'` script
```
...
if npm run build && npm run test; then
    ./.kiro/hooks/send-discord.sh '✅ Build and tests passed - ready to deploy'
else
    ./.kiro/hooks/send-discord.sh '❌ Build or tests failed - deployment blocked'
fi
```
...
{
  "scripts": {
    "deploy": "npm run build && npm run test && ./.kiro/hooks/send-discord.sh '✅ Deployment successful'",
    "notify": "./.kiro/hooks/send-discord.sh"
  }
}
```
...
```bash
./.kiro/hooks/send-discord.sh 'Message with "double quotes" works fine'
```

**Escape single quotes inside single-quoted strings:**
```bash
./.kiro/hooks/send-discord.sh 'User'\''s profile updated'
```

**Use double quotes and escape:**
```bash
./.kiro/hooks/send-discord.sh "Message with \"escaped quotes\""
```

**For complex messages, use heredoc:**
```bash
./.kiro/hooks/send-discord.sh "$(cat <<'EOF'
Multi-line message
...
if npm run test; then
    if [[ "$ENVIRONMENT" == "production" ]]; then
        ./.kiro/hooks/send-discord.sh '✅ Production deployment successful'
    fi
else
    # Always notify on failures
    ./.kiro/hooks/send-discord.sh '❌ Tests failed - deployment aborted'
fi
```
...
**Script Location:** `.kiro/hooks/send-discord.sh`
...
**Kiro Documentation:** https://docs.claude.com/claude-code
