...
## kiro-kit-dev domain hooks

- **Migration Safety Review** — When a DB migration is created, generate the down migration and review for unsafe operations. (`PostFileCreate`)
- **API Contract Sync** — When a route/handler changes, update the OpenAPI spec to match. (`PostFileSave`)
- **Endpoint Test Coverage** — When a new endpoint is added, scaffold integration tests for its success and error paths. (`PostFileCreate`)
