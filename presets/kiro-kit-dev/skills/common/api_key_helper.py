...
def get_api_key_or_exit(skill_dir: Optional[Path] = None) -> str:
    ...
    if not api_key:
        ...
        print("\n3️⃣  .kiro/.env file:", file=sys.stderr)
        print(f"   echo 'GEMINI_API_KEY=your-api-key' > {project_dir}/.kiro/.env", file=sys.stderr)

        print("\n4️⃣  .kiro/skills/.env file (shared across all Gemini skills):", file=sys.stderr)
        print(f"   echo 'GEMINI_API_KEY=your-api-key' > {project_dir}/.kiro/skills/.env", file=sys.stderr)
