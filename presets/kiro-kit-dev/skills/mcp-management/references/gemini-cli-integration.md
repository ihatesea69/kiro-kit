...
Gemini CLI reads MCP servers from `.gemini/settings.json`. Create a symlink to `.kiro/.mcp.json`:
...
ln -sf .kiro/.mcp.json .gemini/settings.json
...
1. **Configuration Loading**: Reads `.gemini/settings.json` (symlinked to `.kiro/.mcp.json`)
...
Should show symlink pointing to `.kiro/.mcp.json`.
