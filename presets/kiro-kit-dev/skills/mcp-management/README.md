...
```bash
cd .kiro/skills/mcp-management/scripts
npm install
```
...
See `.kiro/.mcp.json.example` for more examples.
...
- Load config from `.kiro/.mcp.json`
...
2. `.kiro/skills/mcp-management/.env`
3. `.kiro/skills/.env`
4. `.kiro/.env`
...
The `mcp-manager` agent (`.kiro/agents/mcp-manager.md`) uses this skill to:
...
Ensure `.kiro/.mcp.json` exists and is valid JSON.
