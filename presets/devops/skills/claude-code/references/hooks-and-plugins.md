...
Hooks are configured in `.kiro/hooks.json`:
...
```json
{
  "hooks": {
    "pre-tool": {
      "bash": "./.kiro/scripts/validate-bash.sh"
    }
  }
}
```
...
```json
{
  "hooks": {
    "post-tool": {
      "write": "./.kiro/scripts/format-code.sh",
      "edit": "./.kiro/scripts/format-code.sh"
    }
  }
}
```
...
```bash
#!/bin/bash
# .kiro/scripts/track-usage.sh

# Log prompt
echo "$(date): $USER_PROMPT" >> .kiro/usage.log
...
{
  "hooks": {
    "user-prompt-submit": "./.kiro/scripts/track-usage.sh"
  }
}
```
