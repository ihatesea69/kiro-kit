...
Copy `.kiro/.env.example` to `.kiro/.env` and fill in:
- GITHUB_TOKEN
- DATABASE_URL
```
...
- `.kiro/skills/`
- `.kiro/hooks.json`
- `.kiro/mcp.json` (without secrets)
- `.kiro/.env.example`

**What NOT to commit:**
- `.kiro/.env` (contains secrets)
- `.kiro/memory/` (optional)
- `.kiro/cache/`
- API keys or tokens
...
echo 'ANTHROPIC_API_KEY=sk-ant-xxxxx' > .kiro/.env
```
...
```bash
# Check hooks.json
cat .kiro/hooks.json | jq .

# Review scripts
cat .kiro/scripts/hook.sh
...
```bash
#!/bin/bash
# .kiro/scripts/rate-limit.sh

REQUESTS_FILE=".kiro/requests.log"
MAX_REQUESTS=100
WINDOW=3600  # 1 hour
