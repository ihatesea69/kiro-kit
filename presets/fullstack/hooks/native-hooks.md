...
## fullstack domain hooks

- **Type Sync** — When an API route changes, regenerate shared client types so frontend and backend stay in lockstep. (`PostFileSave`)
- **Env Schema Sync** — When .env.example changes, update the runtime env validation schema. (`PostFileSave`)
...
- `/deployment-readiness` — Run on demand for a pre-deploy checklist across frontend, backend, and infra.
