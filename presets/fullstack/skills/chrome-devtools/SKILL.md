...
description: >-
  ...
  scripts. Use for automating browsers, taking screenshots, analyzing
  performance, monitoring network traffic, web scraping, form automation, and
...
cd .kiro/skills/chrome-devtools/scripts
...
All scripts are in `.kiro/skills/chrome-devtools/scripts/`
...
Screenshots are **automatically compressed** if they exceed 5MB to ensure compatibility with Gemini API and Kiro (which have 5MB limits). This uses ImageMagick internally:
...
2. Verify in `.kiro/skills/chrome-devtools/scripts/` directory
