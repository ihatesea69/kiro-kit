...
**the upstream kit Parity Sync** là một maintainer-time tool (Node.js script) được triển khai như một build-time utility, KHÔNG phải là runtime feature của CLI `kiro-kit`. Mục tiêu duy nhất của nó là rút ngắn khoảng cách content giữa source kit (`the-upstream-kit/.claude/`) và 6 preset self-contained của KiroKit (`presets/{frontend,backend,fullstack,mobile,devops,data-ai}`), đồng thời tôn trọng ràng buộc tailoring per-category đã được phê duyệt trong tài liệu yêu cầu.

Tool này được hiện thực dưới dạng một CLI script chạy thủ công: `node scripts/parity-sync/run.js [--dry-run] [--preset <name>]`. Output bao gồm ba artifact: `delta-report.md`, `conflict-log.md`, `parity-sync-report.md` (tất cả đặt tại `docs/audits/upstream-parity/`). Nó không sửa CLI source, không tạo shared core, không thay đổi public API CLI.
...
**Đầu vào.** `docs/audits/upstream-parity/appendix/`.
...
- Với mỗi `srcItem` × mỗi preset `P` (6 preset chính + `_template`):
  - Chuẩn hoá `srcItem.path` thành `relPath` bằng cách strip prefix `the-upstream-kit/.claude/`.
...
|---|---|---|
| the-upstream-kit/.commitlintrc.json | ./.commitlintrc.json | copy nếu không tồn tại; diff nếu có |
| the-upstream-kit/.releaserc.json | ./.releaserc.json | copy nếu không tồn tại; skip nếu có (CI nhạy cảm) |
| the-upstream-kit/.repomixignore | ./.repomixignore | copy nếu không tồn tại; merge nếu có |
| the-upstream-kit/CLAUDE.md | ./KIRO.md | copy + rebrand (rename) |
| the-upstream-kit/GEMINI.md | ./GEMINI.md | copy nếu không tồn tại |
| the-upstream-kit/guide/ | ./docs/guide/ | copy nếu target rỗng |
| the-upstream-kit/scripts/test-scout-block.{sh,ps1} | ./scripts/test-scout-block.{js,sh,ps1} | copy + sinh `.js` tương đương |
...
- **`tri-script-extend`** áp dụng cho hook `discord_notify.sh` và `telegram_notify.sh` của the upstream kit (single-platform). Vì KiroKit đã có `discord-notify.{js,sh,ps1}` (Req 7.2), planner skip hai file này và chỉ port `discord-hook-setup.md`, `telegram-hook-setup.md` (docs).
...
const SUBSTITUTIONS = [
  ...
  { from: /the upstream kit/g, to: 'KiroKit' },
...
interface SourceItem {
  ...
  path: string;               // "the-upstream-kit/.claude/agents/brainstormer.md"
  basename: string;
  size_lines: number;
  front_matter: {
    present: boolean;
...
interface DeltaEntry {
  ...
  source_path: string;        // relative đã strip "the-upstream-kit/.claude/"
...
```typescript
interface PortPlan {
  source_path: string;            // path tương đối từ the upstream kit root
...
*For all* file output `.md`, `.json`, hoặc script (`.js`, `.sh`, `.ps1`) trong `presets/` và `docs/audits/upstream-parity/`, nội dung không match regex emoji `/[\u{1F300}-\u{1FAFF}\u{2600}-\u{27BF}]/u` và không match PII pattern (email RFC-5322 simplified, phone E.164, real-name placeholder). Nếu PII xuất hiện trong source, content target phải có placeholder (`[email]`, `[phone_number]`, `[name]`) thay vì giá trị thực.
...
- **Tag format:** mỗi property test có comment header:
  ```js
  // Feature: upstream-parity-sync, Property 3: Category Mapping Correctness
  ```
