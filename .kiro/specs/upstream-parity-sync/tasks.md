...
Triển khai theo design tại `.kiro/specs/upstream-parity-sync/design.md`. Mọi task tuân thủ Requirements 18 và 20: KHÔNG sửa `packages/cli/src/`, KHÔNG tạo shared core, KHÔNG tạo preset thứ 7. Dùng Node.js (CommonJS) cho `scripts/parity-sync/`, không cần build step.
...
- [x] 7. Implement `rebrander.js`
  - [x] 7.1 String substitution rules theo design (`.claude/`→`.kiro/`, `the upstream kit`→`KiroKit`, `Claude Code`→`Kiro` với exception list)
...
- [x] 14. Chạy parity-sync `--apply` lần đầu cho 6 preset
  ...
  - [x] 14.4 Commit kết quả: `presets/**`, `docs/audits/upstream-parity/{delta,conflict,parity-sync}-report.md`
...
- [x] 17. Port root-level files (separate from preset content)
  - [x] 17.1 Copy `the-upstream-kit/.commitlintrc.json` → `./.commitlintrc.json` nếu chưa tồn tại
  ...
  - [x] 17.6 Copy `the-upstream-kit/guide/` → `./docs/guide/` nếu target rỗng
