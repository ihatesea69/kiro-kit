...
**the upstream kit Parity Sync** là một feature bổ sung (additive) cho dự án KK-Kiro-Kit nhằm rà soát và lấp các khoảng trống nội dung (content delta) giữa bộ kit tham chiếu **the upstream kit** (`the-upstream-kit/.claude/`) và **6 preset self-contained** hiện có của KiroKit (`presets/{frontend,backend,fullstack,mobile,devops,data-ai}`).

Mục tiêu của feature này là copy + adapt mọi artifact có trong the upstream kit nhưng còn thiếu hoặc chưa đầy đủ trong KiroKit, đồng thời tôn trọng:
...
**Phạm vi bao phủ (Coverage Scope):** Source inventory đã được audit liệt kê **133 artifacts** the upstream kit (`agent=16, command=53, docs_template=12, env_example=3, hook=7, mcp_template=1, metadata=1, settings=1, skill=32, statusline=3, workflow=4`); target inventory KiroKit hiện có **754 file** trải đều trên 7 thư mục preset (`_template=74, backend=107, frontend=110, fullstack=111, mobile=117, devops=115, data-ai=120`). Khoảng cách lớn nhất nằm ở danh mục **command** (KiroKit baseline ~25-28 command per preset vs the upstream kit 53 command) và **skill** (KiroKit ~20 skill per preset vs the upstream kit 32 skill, đa phần thiếu `references/` và `scripts/`).

**Source of Truth:** `docs/audits/upstream-parity/appendix/` (đặc biệt là `inventory-source.json`, `inventory-target.json`, `source-files.txt`, và `target-files-<preset>.txt`). Mọi delta liệt kê trong tài liệu này phải có đường dẫn tham chiếu cụ thể vào audit appendix.
...
- Không port nội dung mang tính branding the upstream kit gốc nguyên văn; mọi tham chiếu "Claude Code" / "the upstream kit" trong nội dung copy phải được rebrand sang "Kiro" / "KiroKit" trong quá trình adapt.
- Không thay đổi spec hiện có `.kiro/specs/kk-kiro-kit/`; spec này là spec mới, độc lập, additive.

## Glossary

- **the upstream kit / Source Kit**: Bộ kit tham chiếu tại `the-upstream-kit/.claude/`, dạng flat single-flavor.
...
- **Audit Appendix**: Thư mục `docs/audits/upstream-parity/appendix/` chứa 7 inventory file và `run.log`; được coi là ground truth cho mọi delta count.
...
- **Rebrand Rule**: Quy tắc thay thế chuỗi `Claude Code` -> `Kiro`, `the upstream kit` -> `KiroKit`, `.claude/` -> `.kiro/`, `claude-code` skill name -> `kiro` skill name (hoặc giữ `claude-code` làm tham chiếu read-only nếu nội dung là docs về Claude Code product).
...
- **Audit Run Log**: `docs/audits/upstream-parity/appendix/run.log` cung cấp count chính thức.
...
1. THE Parity_Sync_Process SHALL đọc `docs/audits/upstream-parity/appendix/inventory-source.json` và bảy file `target-files-*.txt` (`_template`, `backend`, `frontend`, `fullstack`, `mobile`, `devops`, `data-ai`) trước khi sinh ra danh sách file cần port.
2. THE Parity_Sync_Process SHALL sinh ra một file `docs/audits/upstream-parity/delta-report.md` liệt kê, cho mỗi cặp `(source_artifact, preset)`, một trong các trạng thái: `present`, `missing`, `partial`, `category-skip`.
3. WHEN một artifact the upstream kit thuộc category trùng với một preset (ví dụ skill `frontend-development` đối với preset `frontend`), THE Parity_Sync_Process SHALL gán trạng thái `missing` hoặc `partial` thay vì `category-skip` nếu artifact đó vắng mặt hoặc không đủ subdirectory.
...
**User Story:** Là Kiro user, tôi muốn mỗi preset cung cấp đầy đủ bộ agent core của the upstream kit, để tôi không phải tự copy thủ công.

#### Acceptance Criteria

1. THE Parity_Sync_Process SHALL port cả 16 agent từ `the-upstream-kit/.claude/agents/` sang `presets/<preset>/agents/` cho cả 6 preset chính: `brainstormer`, `code-reviewer`, `copywriter`, `database-admin`, `debugger`, `docs-manager`, `git-manager`, `journal-writer`, `mcp-manager`, `planner`, `project-manager`, `researcher`, `scout-external`, `scout`, `tester`, `ui-ux-designer`.
...
**User Story:** Là Kiro user, tôi muốn mỗi preset cung cấp đầy đủ skill (kèm `references/` và `scripts/`) ngang the upstream kit, để các agent có đủ progressive disclosure context.

#### Acceptance Criteria

1. THE Parity_Sync_Process SHALL port toàn bộ 32 skill từ `the-upstream-kit/.claude/skills/` sang `presets/<preset>/skills/` cho cả 6 preset chính, áp dụng quy tắc category tailoring tại Requirement 5.
...
2. WHEN the upstream kit có hook `send-discord.sh` (single-platform), THE Parity_Sync_Process SHALL extend thành tri-script `send-discord.{js,sh,ps1}` trước khi port; nếu logic đã được hợp nhất trong `discord-notify.{js,sh,ps1}` của KiroKit, KHÔNG port `send-discord.*`.
...
7. THE Hook_Env_Example SHALL hợp nhất nội dung từ `the-upstream-kit/.claude/hooks/.env.example` vào `presets/<preset>/hooks/.env.example` mà không xoá biến đã có; trùng key thì giữ giá trị KiroKit.

### Requirement 8: Port Workflows Và Files Cấp Cao

**User Story:** Là Kiro user, tôi muốn mọi workflow always-on (development-rules, primary-workflow, v.v.) đều phản ánh quy tắc the upstream kit mới nhất.
...
4. THE Parity_Sync_Process SHALL port `the-upstream-kit/.claude/metadata.json` schema-relevant fields vào `presets/<preset>/metadata.json` (giữ `kit_version` và `preset_version` của KiroKit, chỉ thêm field mới nếu source có).
5. THE Parity_Sync_Process SHALL diff `the-upstream-kit/.claude/settings.json` với `presets/<preset>/settings.json`; nếu source có hook entry hoặc statusLine command mà target thiếu, hợp nhất với policy "không xoá entry KiroKit-specific".
...
**User Story:** Là Kiro user, tôi muốn statusline hoạt động đa nền tảng giống the upstream kit.
...
2. WHEN nội dung statusline the upstream kit khác KiroKit, THE Parity_Sync_Process SHALL diff và áp dụng Conflict Resolution (Requirement 12); ưu tiên giữ logic KiroKit nếu phong phú hơn.
...
**User Story:** Là maintainer KiroKit, tôi muốn các file chuẩn hoá quy trình release/git của the upstream kit cũng có ở repository root để giữ tooling parity.

#### Acceptance Criteria

1. THE Parity_Sync_Process SHALL evaluate copy các file root sau từ `the-upstream-kit/` vào root KiroKit nếu chưa tồn tại: `.commitlintrc.json`, `.releaserc.json`, `.repomixignore`, `CLAUDE.md` (rename thành `KIRO.md`), `GEMINI.md`.
2. THE Parity_Sync_Process SHALL evaluate copy thư mục `the-upstream-kit/guide/` vào `docs/guide/` nếu source có nội dung và target chưa có.
3. THE Parity_Sync_Process SHALL evaluate copy `the-upstream-kit/scripts/test-scout-block.sh` và `test-scout-block.ps1` vào `scripts/` của KiroKit, kèm bản `.js` tương đương để tuân thủ tri-script.
...
1. THE Rebrand_Rule SHALL áp dụng các thay thế sau khi port nội dung từ source sang target:
   ...
   - Chuỗi `the upstream kit` -> `KiroKit`.
...
5. THE Conflict_Resolution SHALL ghi log mọi quyết định vào `docs/audits/upstream-parity/conflict-log.md`.
...
1. WHEN parity sync hoàn tất cho tất cả 6 preset, THE Parity_Sync_Process SHALL sinh `docs/audits/upstream-parity/parity-sync-report.md` chứa: tổng số file đã port, số file đã skip (kèm lý do), số conflict đã giải quyết, số manual-review pending.
...
5. THE Parity_Sync_Spec SHALL không port nội dung mang tính branding gốc của the upstream kit (logo, screenshot, marketing copy) vào KiroKit.
